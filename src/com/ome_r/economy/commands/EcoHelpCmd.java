...
@Command(name = "help", aliases = "", args = "[page#] ", perm = "prisoncore.economy",
			desc = "Shows the list of commands of this plugin.", minArgs = 0, maxArgs = 1)
public class EcoHelpCmd extends CommandsHandle{
	...
	public void run(Player p, String[] args) {
		...
		for(CommandsHandle c : cmds){
			...
			commands += getMessage("COMMANDS_FORMAT", p).replace("{0}", "eco").replace("{1}", data.name())
					.replace("{2}", data.desc() + "\n");
		}
		
		p.sendMessage(getMessage("COMMANDS_LIST", p).replace("{0}", "Economy").replace("{1}", page + "")
				.replace("{2}", maxPages + "").replace("{3}", commands).replace("{4}", "eco"));
	}
